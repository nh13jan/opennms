...
public final class EventTranslatorConfigFactory implements EventTranslatorConfig {
     ...
     * 
     */
    private EventTranslatorConfigFactory(final String configFile, final DataSource dbConnFactory) throws IOException, MarshalException, ValidationException {
     ...
     */
    public EventTranslatorConfigFactory(final InputStream rdr, final DataSource dbConnFactory) throws MarshalException, ValidationException {
        unmarshall(rdr, dbConnFactory);
    }

    private void unmarshall(final InputStream stream, final DataSource dbConnFactory) throws MarshalException, ValidationException {
    ...
    private void unmarshall(final InputStream stream) throws MarshalException, ValidationException {
    ...
    public void update() throws Exception  {
        ...
        try {
            final File cfgFile = ConfigFileConstants.getFile(ConfigFileConstants.TRANSLATOR_CONFIG_FILE_NAME);
    ...
    public static synchronized EventTranslatorConfig getInstance() {
        if (!m_loaded) throw new IllegalStateException("getInstance: The factory has not been initialized");
        return m_singleton;
    }
     ...
     */
    public static synchronized void setInstance(final EventTranslatorConfig singleton) {
     ...
     * @return the PassiveStatus configuration
     */
    private EventTranslatorConfiguration getConfig() {
    ...
    private List<String> getTranslationUEIs() {
        m_lock.readLock().lock();
        try {
            final Translation translation = getConfig().getTranslation();
            if (translation == null) return Collections.emptyList();
    
            final List<String> ueis = new ArrayList<String>();
            for (final EventTranslationSpec event : translation.getEventTranslationSpecCollection()) {
    ...
    /** {@inheritDoc} */
    @Override
    public boolean isTranslationEvent(final Event e) {
        m_lock.readLock().lock();
        try {
            for (final TranslationSpec spec : getTranslationSpecs()) {
                if (spec.matches(e))
                    return true;
            }
            return false;
    ...
    /** {@inheritDoc} */
    @Override
    public List<Event> translateEvent(final Event e) {
        m_lock.readLock().lock();
        try {
    ...
    ValueSpec getValueSpec(final Value val) {
    ...
    class SqlValueSpec extends ValueSpec {
        ...
        public Query createQuery(final Event srcEvent) {
            m_lock.readLock().lock();
            try {
                final Object[] args = new Object[getNestedValues().size()];
    ...
    static class TranslationFailedException extends RuntimeException {
        private static final long serialVersionUID = 6005280374243293115L;
        TranslationFailedException(final String msg) {
            super(msg);
        }
    }
}
